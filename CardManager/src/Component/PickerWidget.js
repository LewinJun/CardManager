...
const [aWidth, aHeight] = [width, 260];
const [left, top] = [0, 0];
...
export default class PickerWidget extends Component {
  ...
  constructor(props) {
    ...
    if (this.props.defaultVal === null || this.props.defaultVal === undefined && this.options.length > 0) {
        ...
        <View style={styles.container} >
          ...
          <Animated.View style={[styles.tip, {
            ...
            <Picker
              ...
              {this.state.options.map((aOption) => <Picker.Item color='#b5b9be' label={aOption} value={aOption} key={aOption} />)}
            </Picker>
          </Animated.View>
        </View>
      );
  ...
  ok() {
    if (!this.state.hide) {
      this.out();
      if (this.callback) {
        this.callback.apply(this.parent, [this.state.choice,this.parent]);
      }

    }
  }

  show(obj: Object, callback: Object ,options:Array) {
    this.parent = obj;
    this.callback = callback;
    if (options) {
      this.options = options;
      this.setState({ options: options,choice:options[0] });
    }
    if (this.state.hide) {
